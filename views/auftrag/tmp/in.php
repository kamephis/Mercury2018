<?php
    print_r($_REQUEST);
    $listSize   = sizeof($this->auftrag->getAuftrag(Session::get('artEAN'))); // Anzahl Positionen

    // Anlegen eines neuen Auftrags in der Datenbank
    $userID = Session::get('UID');

    // Wenn eine EAN zum 1. Mal von ScanArt übergeben wurde
    if($_GET['eanScanned'] == 1){
        // Zurücksetzen des Meter Zählers (Session) - Anzeige auf ScanArt
        Session::set('geschnitteneMeterGesamt', 0);

        // Nur einen Auftrag erstellen, wenn es dafür Positionen gibt
    ...
    if ($listSize > 0){
        ...
        if ($this->auftrag->getAuftrag(Session::get('artEAN'))) {
    ...
    // Auslesen der neuen Auftragsnummer - MAX ID
    $aNr = $this->auftrag->getAuftragsnummer();

    // Speichern der bearbeiteten Position
    if($_POST['savePos']){
        // Addieren der bereits geschnittenen Meter
